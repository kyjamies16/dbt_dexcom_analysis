...
def get_date_range_selector(df):
  ...
  predefined_options = {
    "Last 7 Days": (yesterday - datetime.timedelta(days=7), datetime.date.today()),
    "Last 30 Days": (yesterday - datetime.timedelta(days=30), datetime.date.today()),
    "This Month": (yesterday.replace(day=1), datetime.date.today()),
    "Last 90 Days": (yesterday - datetime.timedelta(days=90), datetime.date.today()),
    "All Time": (
  ...
  if choice == "Custom Range":
    ...
    custom_range = st.sidebar.date_input(
      "Pick a custom range:",
      value=(datetime.date.today() - datetime.timedelta(days=7), yesterday),
      min_value=df["reading_timestamp"].min().date(),
      max_value=min(df["reading_timestamp"].max().date(), yesterday),
    )
