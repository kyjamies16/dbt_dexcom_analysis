...
def generate_glucose_time_chart(df: pd.DataFrame, start_date, end_date):
  ...
  y_domain = [0, max(220, agg["p75"].max() + 10)]
...
def generate_daily_average_chart(df: pd.DataFrame, start_date, end_date):
    """
    Shows daily average glucose with 7-day rolling average and labeled legend.
    """
    if df.empty:
        return None
    ...
    daily_avg_df["rolling_avg"] = daily_avg_df["avg_glucose"].rolling(7).mean()

    # Step 3: Filter dates for display
    ...
    # Step 4: Reshape for combined line chart
    melted_df = pd.concat([
        filtered_df[["date", "avg_glucose"]].rename(columns={"avg_glucose": "glucose"}).assign(label="Daily Avg"),
        filtered_df[["date", "rolling_avg"]].rename(columns={"rolling_avg": "glucose"}).assign(label="7-Day Rolling Avg")
    ])
    ...
    chart = alt.Chart(melted_df).mark_line(point=True).encode(
        ...
        y=alt.Y("glucose:Q", title="Avg Glucose (mg/dL)", scale=alt.Scale(domain=y_domain)),
